...
Info
    Engine orchestrating the initialization and execution of picosign.
'''
...
class PicoSign(Base):
    '''
        ...
        It defines:
            ...
            :attributes:
                | _is_initialized - The flag indicating whether the picosign engine is initialized.
            ...
            :methods:
                | __init__ - Initializes the picosign engine with adapters and services.
                | process - Processes the picosign commands.
    '''
    ...
    def __init__(self, bundle: PicoSignBundle) -> None:
        '''
            Initializes the picosign engine with adapters and services.

            :param bundle: PicoSign bundle containing adapters and services.
            :exceptions: None.
        '''
        ...
        try:
            ...
            self._logger.write_log(INFO, '✅ picosign: engine initialized successfully!')

        except (ATSValueError, ATSTypeError) as exc:
            stdout.write(f'❌ picosign: {exc}!\n')

        except Exception as exc:
            stdout.write(f'❌ picosign unexpected exception: {exc}!\n')

    def process(self, verbose: bool = False) -> bool:
        '''
            Processes the picosign commands.
        ...
        try:
            if self.is_initialized():
                ...
                if result.get('returncode') != 0:
                    self._logger.write_log(ERROR, f'❌ picosign: {result.get("stderr") or "failed!"}')
                    return False

                self._logger.write_log(INFO, '✅ picosign: done!')
                self._logger.write_log(INFO, '✅ picosign: exiting successfully!')
                return True

            self._logger.write_log(ERROR, '❌ picosign: engine not initialized!')
            return False

        except (ATSValueError, ATSTypeError) as exc:
            self._logger.write_log(ERROR, f'❌ picosign: {exc}!')
            return False

        except Exception as exc:
            self._logger.write_log(ERROR, f'❌ picosign unexpected exception: {exc}!')
            return False
